jQuery ->
  ...
  window.youtubePlayerLoaded = false
  ...
  makeVideoPlayer = (video) ->
    if !window.youtubePlayerLoaded
      ...
      window.youtube_player = new YT.Player('ytPlayer', {
        ...
        events: {
          'onReady': -> window.youtubePlayerLoaded = true
          'onError': (errorCode) -> alert("We are sorry, but the following error occured: " + errorCode)
        }
      })
    else
      window.youtube_player.loadVideoById(video)
